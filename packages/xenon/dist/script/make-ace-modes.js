...
var blacklist = ['c_cpp', 'css', 'golang', 'javascript', 'json', 'plain_text', 'c9search', 'jsx', 'php', 'snippets'];
...
for (var i = 0; i < modelist.modes.length; i++) {
    ...
    for (var ei = 0; ei < extensions.length; ei++) {
        ...
        if (ext[0] === '^') filenames_pretty.push('                "' + ext.substr(1) + '"');else extensions_pretty.push('                "' + ext + '"');
    }

    fs.writeFileSync(jsonName, '{\n' + '    modes: {\n' + '        ' + mode.name + ': {\n' + '            name: "' + mode.caption + '",\n' + '            highlighter: "' + mode.mode + '",\n' + (filenames_pretty.length ? '            filenames: [\n' + filenames_pretty.join(',\n') + '\n' + (extensions_pretty.length ? '            ],\n' : '            ]\n') : '') + (extensions_pretty.length ? '            extensions: [\n' + extensions_pretty.join(',\n') + '\n' + '            ]\n' : '') + '        }\n' + '    }\n' + '}\n', { flags: 'wx' });

    modesJson.imports.push('./mode/' + mode.name + '.json');
}

modesJson.imports.sort();
fs.writeFileSync('app/config/default/modes.json', JSON.stringify(modesJson, null, 4));
//# sourceMappingURL=make-ace-modes.js.map