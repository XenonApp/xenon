...
async function update(name) {
    const [newest, json] = await Promise.all([utils.view(name, 'version'), configfs.readFile(`/node_modules/${name}/package.json`)]);
...
async function updateAll() {
    ...
    for (const name of packageNames) {
        const promise = Promise.all([utils.view(name, 'version'), configfs.readFile(`/node_modules/${name}/package.json`)]).then(([newest, json]) => {
            if (!newest || !json) {
                return false;
            }
