...
function defineUserCommand(name, cmd) {
    api.defineConfig(name, {
        ...
        exec: function (edit, session) {
            return require("./sandboxes").execCommand(name, cmd, session).catch(function (err) {
...
var api = {
    ...
    exec: function (path, edit, session) {
        ...
        if (!session.getTokenAt) {
            // Check if this is a session object
...
api.define("Help:Commands", {
    exec: function (edit, session) {
        return require("./session_manager").go("zed::commands", edit, session).then(function (session) {
            var command_list = "> Zed Online Command Reference.\n" + "\n   What follows is a complete reference of all commands " + "known to Zed, and their current keybindings, even if " + "you've modified the defaults. Click a command to activate it.\n\n" + "   `Fold:Fold All`\n   `Fold:Unfold All`\n\n\n";
            var prev_tree = [];
            api.allCommands().sort().forEach(function (command_name) {
                ...
                for (var i = 0; i < len; ++i) {
                    if (prev_tree[i] !== command_tree[i]) {
                        command_list += "\n" + new Array(i + 2).join("#") + " " + command_tree[i] + "\n\n";
                    }
                }
