...
var api = {
    hook: function () {
        eventbus.on("configchanged", function (config) {
            ...
            Promise.all([setEditorTheme(editorTheme), setWindowTheme(windowTheme), loadUserCss()]).then(function () {
                refreshScrollBars();
            });
        });
        ...
        eventbus.on("configcommandsreset", function (config) {
            ...
            _.each(allEditorThemes, function (theme, name) {
                command.defineConfig("Configuration:Editor Theme:" + theme.name, {
                    doc: "Activate this editor theme. Change will take effect " + "immediately, and also persist in Zed's configuration file.",
                    exec: function () {
            ...
            _.each(allWindowThemes, function (theme, name) {
                command.defineConfig("Configuration:Window Theme:" + theme.name, {
                    doc: "Activate this window theme. Change will take effect " + "immediately, and also persist in Zed's configuration file.",
                    exec: function () {
